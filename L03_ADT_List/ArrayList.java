...
public class ArrayList<T> implements List<T> {
    ...
    public void insert(T e) {
        for (int i = size; i > current; i--) {
            nodes[i] = nodes[i - 1];  // Shift elements to the right to make space for the new element at the current index.
        }
    ...
    public void remove() {
        for (int i = current; i < size; i++) {
            nodes[i] = nodes[i + 1];  // Shift elements to the left to remove the element at the current index.
        }
